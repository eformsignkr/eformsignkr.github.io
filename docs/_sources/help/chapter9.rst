...
**④ 필드 추가**
   새로운 필드를 추가합니다. **필드 추가** 버튼을 클릭하면 사용자 정의 필드를 추가할 수 있는 팝업창이 나타납니다. **필드 이름** 과 **필드 타입** (텍스트/ 이미지), **필드 값**\ 을 입력한 다음 확인을 클릭하면 목록에 필드가 추가됩니다.
...
**⑥ 필드 편집**
   수정할 필드를 선택하여 수정 아이콘(|image1|)을 클릭하면 사용자 정의 필드 수정 팝업 창이 표시되어 필드 이름, 타입, 값 등을 수정할 수 있습니다.
...
4. 이메일 제목 및 SMS 전송 내용을 입력합니다. 
   **SMS 전송 내용**\ 은 문자로 알림 전송 시 전송되는 메시지로 알림 본문 내용으로 이동할 수 있는 URL과 함께 전송됩니다. 
